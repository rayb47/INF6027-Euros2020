...
create_boxplot <- function(data, field, group_var, seed = 123) {
  ...
  ggplot(data, aes(x = .data[[group_var]], y = .data[[field]], fill = .data[[group_var]])) +
    ...
    labs(
      title = paste(field, ": Group Stage vs Knockouts"),
      x = group_var,
      y = field
    ) +
    theme_minimal()
...
plot_stage_comparison <- function(data_long, metric_name) {
  ...
  ggplot(filtered_data, aes(x = TeamName, y = Value, fill = Stage)) +
    ...
    labs(
      title = paste("Stage-Wise Comparison for", metric_name),
      x = "Team",
      y = "Metric Value",
      fill = "Stage"
    ) +
...
plot_correlation_analysis <- function(data, title = "Feature Importance Based on Correlation with Qualification") {
